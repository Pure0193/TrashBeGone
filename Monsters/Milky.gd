...
onready var _milky_drop = preload("res://Item/MilkDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var milky_drop = _milky_drop.instance()
		get_parent().add_child(milky_drop)
		milky_drop.global_position = self.global_position
	queue_free()
