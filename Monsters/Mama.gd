...
onready var _mama_drop = preload("res://Item/MamaDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var mama_drop = _mama_drop.instance()
		get_parent().add_child(mama_drop)
		mama_drop.global_position = self.global_position
	queue_free()
