...
onready var _charger_drop = preload("res://Item/ChargerDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var charger_drop = _charger_drop.instance()
		get_parent().add_child(charger_drop)
		charger_drop.global_position = self.global_position
	queue_free()
