...
onready var _lightbulb_drop = preload("res://Item/LightbulbDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var lightbulb_drop = _lightbulb_drop.instance()
		get_parent().add_child(lightbulb_drop)
		lightbulb_drop.global_position = self.global_position
	queue_free()
