...
onready var _cabbage_drop = preload("res://Item/CabbageDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var cabbage_drop = _cabbage_drop.instance()
		get_parent().add_child(cabbage_drop)
		cabbage_drop.global_position = self.global_position
	queue_free()
