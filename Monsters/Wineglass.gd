...
onready var _winedrop = preload("res://Item/WineDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var winedrop = _winedrop.instance()
		get_parent().add_child(winedrop)
		winedrop.global_position = self.global_position
	queue_free()
