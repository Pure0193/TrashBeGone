...
onready var _strange_drop = preload("res://Item/Key5Drop.tscn")
...
func _on_Stats_dead():
	...
	for item_index in PlayerInventory.inventory:
		if "Key5" in String(PlayerInventory.inventory[item_index][0]):
			is_key += 1
	if is_key == 0:
		var strange_drop = _strange_drop.instance()
		get_parent().add_child(strange_drop)
		strange_drop.global_position = self.global_position
	queue_free()

func _on_Hitbox_area_entered(area):
	animationState.travel("StrangeAttack")
	animationTimer.start()


func _on_Timer_timeout():
	animationState.travel("StrangeIdle")
