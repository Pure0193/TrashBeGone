...
onready var _dino_drop = preload("res://Item/Key2Drop.tscn")
...
func chase_player():
	if DetectionZone.PlayerDetected():
		state = CHASING
		animationState.travel("DinoWalk")
...
func _on_Stats_dead():
	...
	for item_index in PlayerInventory.inventory:
		if "Key2" in String(PlayerInventory.inventory[item_index][0]):
			is_key += 1
	if is_key == 0:
		var dino_drop = _dino_drop.instance()
		get_parent().add_child(dino_drop)
		dino_drop.global_position = self.global_position
	queue_free()

func _on_Hitbox_area_entered(area):
	animationState.travel("DinoAttack")
	animationTimer.start()


func _on_Timer_timeout():
	animationState.travel("DinoIdle")
