...
onready var _slime_drop = preload("res://Item/Key3Drop.tscn")
...
func _on_Stats_dead():
	...
	for item_index in PlayerInventory.inventory:
		if "Key3" in String(PlayerInventory.inventory[item_index][0]):
			is_key += 1
	if is_key == 0:
		var slime_drop = _slime_drop.instance()
		get_parent().add_child(slime_drop)
		slime_drop.global_position = self.global_position
	queue_free()

func _on_Hitbox_area_entered(area):
	animationState.travel("SlimeAttack")
	animationTimer.start()


func _on_Timer_timeout():
	animationState.travel("SlimeIdle")
