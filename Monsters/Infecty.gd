...
onready var _atkdrop = preload("res://Item/ATKDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var atkdrop = _atkdrop.instance()
		get_parent().add_child(atkdrop)
		atkdrop.global_position = self.global_position
	queue_free()
