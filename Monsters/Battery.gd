...
onready var _batterydrop = preload("res://Item/BatteryDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var batterydrop = _batterydrop.instance()
		get_parent().add_child(batterydrop)
		batterydrop.global_position = self.global_position
	queue_free()
