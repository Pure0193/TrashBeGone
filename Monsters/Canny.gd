...
onready var _can_drop = preload("res://Item/CanDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var can_drop = _can_drop.instance()
		get_parent().add_child(can_drop)
		can_drop.global_position = self.global_position
	queue_free()
