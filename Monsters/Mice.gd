...
onready var _mouse_drop = preload("res://Item/MouseDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var mouse_drop = _mouse_drop.instance()
		get_parent().add_child(mouse_drop)
		mouse_drop.global_position = self.global_position
	queue_free()
