...
onready var _spraydrop = preload("res://Item/SprayDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var spraydrop = _spraydrop.instance()
		get_parent().add_child(spraydrop)
		spraydrop.global_position = self.global_position
	queue_free()
