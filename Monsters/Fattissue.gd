...
onready var _tissuedrop = preload("res://Item/TissueDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var tissuedrop = _tissuedrop.instance()
		get_parent().add_child(tissuedrop)
		tissuedrop.global_position = self.global_position
	queue_free()
