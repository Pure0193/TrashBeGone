...
onready var _waterdrop = preload("res://Item/WaterDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var waterdrop = _waterdrop.instance()
		get_parent().add_child(waterdrop)
		waterdrop.global_position = self.global_position
	queue_free()
