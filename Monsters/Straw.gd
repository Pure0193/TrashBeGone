...
onready var _strawdrop = preload("res://Item/StrawDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var strawdrop = _strawdrop.instance()
		get_parent().add_child(strawdrop)
		strawdrop.global_position = self.global_position
	queue_free()
