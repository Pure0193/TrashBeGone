...
onready var _snackdrop = preload("res://Item/SnackDrop.tscn")
...
func _on_Stats_dead():
	randomize()
	if randf() > 0.3:
		var snackdrop = _snackdrop.instance()
		get_parent().add_child(snackdrop)
		snackdrop.global_position = self.global_position
	queue_free()
